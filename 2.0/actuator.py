...
from pins import OutputPin, PwmPin, PinState
...
class PCA9685:
    ...
    def set_duty_cycle(self, duty_cycle):
        if duty_cycle < 0 or duty_cycle > 1:
            ...
            duty_cycle = utils.clamp(duty_cycle, 0, 1)
...
class L298N_HBridge_2pin(object):
    ...
    def run(self, throttle:float) -> None:
        ...
        if throttle > 1 or throttle < -1:
            ...
            throttle = utils.clamp(throttle, -1, 1)

        self.speed = throttle
        self.throttle = utils.map_range_float(throttle, -1, 1, -self.max_duty, self.max_duty)
...
class RPi_GPIO_Servo(object):
    ...
    def run(self, pulse):
        ...
        self.throttle = utils.map_range(pulse, -1.0, 1.0, self.min, self.max)
...
class ArduinoFirmata:
    ...
    def __init__(self, servo_pin = 5, esc_pin = 6):
...
class ArdPWMSteering:
    ...
    def check_integrity(self):
        attributes = ['angle', 'LEFT_ANGLE', 'RIGHT_ANGLE', 'left_pulse', 'right_pulse']
        for attr in attributes:
            if getattr(self, attr, None) is None:
                if attr == 'angle':
                    self.angle = 90
                else:    
                    raise ValueError(f"\n{attr} in {self.__class__.__name__} is None!\n")
...
class ArdPWMThrottle:
    ...
    def check_integrity(self):
        attributes = ['throttle','MIN_THROTTLE', 'MAX_THROTTLE', 'max_pulse', 'min_pulse', 'zero_pulse']
        for attr in attributes:
            if getattr(self, attr, None) is None:
                raise ValueError(f"\n{attr} in {self.__class__.__name__} is None!\n")
