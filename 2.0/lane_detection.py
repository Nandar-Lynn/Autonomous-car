...
#     mid_width = .3 #changed this value - seemed to work a lot better than 0.08 
#     height_pct = .5
#     bottom_trim = .935
...
def process_image(image):
    ...
    leftx = window_centroids[0][0]
    rightx = window_centroids[0][1]

    # Draw a dot at the center of the lane on the original undistorted image
    dot_color = (0, 255, 0)  # Green color for the dot
    cv2.circle(img, (int((leftx + rightx) / 2), int(img.shape[0] * 0.7)), 10, dot_color, -1)
...
while(cap.isOpened()):
    _, frame = cap.read()
    combo_image = process_image(frame)
