...
def check_integrity(x, X_min, X_max, Y_min, Y_max):
    attributes = ['x', 'X_min', 'X_max', 'Y_min', 'Y_max']
    for attr in attributes:
        if locals().get(attr) is None: 
            raise ValueError(f"\n{attr} is None!\n")
...
def map_range(x, X_min, X_max, Y_min, Y_max):
    ...
    check_integrity(x, X_min, X_max, Y_min, Y_max)  # Call the function with the variables
